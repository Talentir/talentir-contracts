...
async function main (): Promise<void> {
  ...
  const TalentirMarketplace = await ethers.getContractFactory('TalentirMarketplaceV0')
  const talentirMarketplace = await TalentirMarketplace.deploy(nftAddress, 0)
  ...
  await verifyEtherscan(talentirMarketplace.address, talentirMarketplace.deployTransaction, [nftAddress, '0'])
}
