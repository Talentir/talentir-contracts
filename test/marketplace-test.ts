...
describe("Marketplace Tests", function () {
  ...
  it("should handle multiple orders", async function () {
    ...
    expect(
      await marketplace
        .connect(buyer)
        .makeBuyOrder(tokenId, 1, false, { value: oneEther.mul(10) })
    )
      .to.emit(marketplace, "OrderExecuted")
      .to.changeEtherBalances(
        [marketplace.address, seller.address],
    ...
    expect(
      ...
      .to.changeEtherBalances(
        [marketplace.address, seller.address],
        [-oneEther.mul(17), oneEther.mul(17)]
      );
    bestOrderId = await marketplace.getBestOrder(tokenId, BUY);
    bestOrder = await marketplace.orders(bestOrderId[0]);
    expect(await bestOrder.quantity).to.equal(1);
    ...
    expect(
      await marketplace
        .connect(buyer)
        .makeBuyOrder(tokenId, 2, false, { value: oneEther.mul(20) })
    )
      .to.emit(marketplace, "OrderExecuted")
      .to.changeEtherBalances(
        [marketplace.address, seller.address],
  ...
  it("should pause and cancel", async function () {
    ...
    expect(await marketplace.makeSellOrder(tokenId, oneEther, 1, true)).to.emit(
      marketplace,
      "OrderAdded"
    );
    ...
    expect(
      ...
      .to.emit(marketplace, "OrderAdded")
      .to.changeEtherBalances([marketplace.address, owner.address], [1, -1]);
    ...
    expect(await marketplace.cancelOrders([1, 2]))
    ...
    expect(marketplace.connect(seller).unpause()).to.be.revertedWith(
      "Ownable: caller is not the owner"
    );
    // Unpause contract
    expect(await marketplace.unpause()).to.emit(marketplace, "Unpaused");
