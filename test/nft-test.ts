...
describe.only('TalentirNFT', function () {
  ...
  it("Uri", async function () {
    ...
    await talentir.setMinterRole(minter.address);
    await talentir.connect(minter).mint(luki.address, cid1, contentID1, luki.address)
  ...
  it("Test Minting / Burning", async function () {
    ...
    await expect(talentir.mint(luki.address, cid1, contentID1, luki.address)).to.be.revertedWith('Not allowed')
    
    await expect(talentir.connect(luki).setMinterRole(minter.address))
      .to.be.revertedWith("Ownable: caller is not the owner")
    await talentir.setMinterRole(minter.address);

    await expect(talentir.connect(minter).mint(luki.address, cid1, contentID1, luki.address))
    ...
    await expect(talentir.connect(luki).burn(luki.address, tokenID1, 1000)).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it('Marketplace address', async function () {
    ...
    await talentir.setMinterRole(minter.address);
    await talentir.connect(minter).mint(luki.address, '', contentID1, ethers.constants.AddressZero)

    await expect(talentir.connect(johnny).safeTransferFrom(luki.address, johnny.address, tokenID1, 1, '0x'))
      .to.be.revertedWith("ERC1155: caller is not token owner or approved")

    await expect(talentir.connect(luki).setNftMarketplaceApproval(johnny.address, true)).to.be.revertedWith("Ownable: caller is not the owner")

    await talentir.setNftMarketplaceApproval(johnny.address, true)

    await talentir.connect(johnny).safeTransferFrom(luki.address, johnny.address, tokenID1, 1, '0x')
  ...
  it('Royalties', async function () {
    ...
    await expect(talentir.connect(minter).mint(luki.address, '', contentID, johnny.address))
    ...
    await expect(talentir.connect(luki).royaltyInfo(1, 100))
      .to.be.reverted

    await expect(talentir.connect(minter).setRoyalty(15_000)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(talentir.setRoyalty(101_000)).to.be.revertedWith("Must be <= 100%")
    ...
    await expect(talentir.connect(luki).updateTalent(tokenID1, luki.address))
      .to.be.reverted
    ...
    it('Pause', async function () {
      ...
      await expect(talentir.connect(johnny).safeTransferFrom(johnny.address, luki.address, tokenID1, 1, '0x'))
        .to.be.revertedWith('Pausable: paused')
  ...
  it("Set Talent", async function () {
    ...
    await expect(talentir.updateTalent(tokenID1, luki.address))
      .to.be.revertedWith("Royalty receiver must update")
